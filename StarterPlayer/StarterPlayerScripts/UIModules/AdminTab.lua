...
function AdminTab.Initialize(parentFrame)
	...
	local splitLayout = Instance.new("UIListLayout", SplitFrame)
	splitLayout.FillDirection = Enum.FillDirection.Horizontal
	splitLayout.Padding = UDim.new(0, 20)
	...
	lpStroke.Color = Color3.fromRGB(45, 45, 50)
	...
	rpStroke.Color = Color3.fromRGB(45, 45, 50)
	...
	local function CreateActionRow(labelText, inputPlaceholders, btnText, btnColorHex, callback)
		local row = Instance.new("Frame", ActionScroll)
		row.Size = UDim2.new(1, -10, 0, 40)
		row.BackgroundTransparency = 1

		local lbl = UIHelpers.CreateLabel(row, labelText, UDim2.new(0, 140, 1, 0), Enum.Font.GothamBold, UIHelpers.Colors.TextWhite, 14)
		...
		local inputBoxes = {}
		for _, placeholder in ipairs(inputPlaceholders) do
			local box = CreateInputBox(row, placeholder, UDim2.new(0, 150, 0, 30))
			table.insert(inputBoxes, box)
		end

		local btn, stroke = UIHelpers.CreateButton(row, btnText, UDim2.new(0, 130, 0, 30), Enum.Font.GothamBlack, 12)
		btn.TextColor3 = Color3.fromHex(btnColorHex:gsub("#", ""))
		stroke.Color = Color3.fromHex(btnColorHex:gsub("#", ""))

		btn.MouseButton1Click:Connect(function()
			if not selectedPlayerName then return end
			local args = {}
			for _, box in ipairs(inputBoxes) do table.insert(args, box.Text) end
			callback(selectedPlayerName, unpack(args))
		end)
	end

	-- [[ HOOKED DIRECTLY TO DATAMANAGER'S EXPECTED COMMANDS ]]
	CreateActionRow("Modify Currency", {"Amount"}, "SET DEWS", "#FF88FF", function(targetName, amount)
		AdminCommand:FireServer("SetDews", targetName, amount)
	end)

	CreateActionRow("Modify EXP", {"Amount"}, "SET XP", "#55FF55", function(targetName, amount)
		AdminCommand:FireServer("SetXP", targetName, amount)
	end)

	CreateActionRow("Set Lineage", {"Clan Name"}, "SET CLAN", "#55AAFF", function(targetName, clanName)
		AdminCommand:FireServer("SetClan", targetName, clanName)
	end)

	CreateActionRow("Set Titan", {"Titan Name"}, "SET TITAN", "#FF5555", function(targetName, titanName)
		AdminCommand:FireServer("SetTitan", targetName, titanName)
	end)

	-- DataManager's GiveItem expects a table with {Item, Amount}
	CreateActionRow("Give Item", {"Item Name", "Amount"}, "GIVE ITEM", "#FFD700", function(targetName, itemName, amount)
		AdminCommand:FireServer("GiveItem", targetName, {Item = itemName, Amount = amount})
	end)

	CreateActionRow("Danger Zone", {}, "WIPE PLAYER DATA", "#FF0000", function(targetName)
		AdminCommand:FireServer("WipePlayer", targetName)
	end)

	acLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
		ActionScroll.CanvasSize = UDim2.new(0, 0, 0, acLayout.AbsoluteContentSize.Y + 10)
	end)
